...
struct UserInfoResponse: Codable {
    let boxId: String
    let boxState: String? //TODO: Remove the optional after Dor fixes the getInfo !!!!
...
class UserManager: UserManagerProtocol {
    ...
    func getUserInfo(success:@escaping () -> Void, failure:@escaping (_ error:Error?, _ response: AnyObject?) -> Void) {
        ...
        getUserInfoFromServer(username: userVM.email, password: userVM.password, success: { response in
            ...
            self.userViewModel = UserViewModel(email: userVM.email, password: userVM.password, boxId: response.boxId, ebayConnection: response.ebayConnection, boxState: response.boxState, boxBaseline: response.boxBaseline)
            success()
        }) { error, response in
